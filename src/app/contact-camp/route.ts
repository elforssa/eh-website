...
const ALLOWED_SOURCES = ['galerie-cta', 'galerie-banniere', 'carte', 'whatsapp'] as const;
type Source = (typeof ALLOWED_SOURCES)[number];

// Link-preview crawlers fetch the URL server-side (e.g. when we paste the link
// into WhatsApp), which would otherwise create fake click rows. Skip logging
...
export async function GET(req: NextRequest) {
  const ctaUrl = process.env.WHATSAPP_CTA_URL;
  if (!ctaUrl) {
    // Fail loudly: a silent fallback would hide a misconfiguration that sends
    // everyone clicking the enrollment CTA to the wrong place.
    console.error('WHATSAPP_CTA_URL is not set — /contact-camp is redirecting to "/contact".');
  ...
  if (userAgent && !BOT_UA.test(userAgent)) {
    ...
    try {
      ...
      console.error('cta_clicks log failed:', err);
    }
  }

  return NextResponse.redirect(ctaUrl, 302);
}
