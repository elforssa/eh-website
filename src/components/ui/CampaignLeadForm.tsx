...
export function CampaignLeadForm({ campaign }: { campaign: CampaignConfig }) {
  ...
  const visible = (campaign.visibleQuestions || []).filter((key): key is Question => key in questions && allowed.has(key as Question));
  ...
  return <form onSubmit={submit} className="space-y-5" noValidate>
    ...
    {visible.map((key) => <label className="block" key={key}>{questions[key]}
